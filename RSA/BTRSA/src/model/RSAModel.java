...
import java.math.BigInteger;//phép làm việc với số nguyên lớn,
import java.util.Random;//tạo số ngẫu nhiên.

public class RSAModel {

	private BigInteger p, q;//ai số nguyên tố lớn ngẫu nhiên 
	private BigInteger n;//hành phần modulus của khóa công khai và khóa bí mật.
	private BigInteger phiN;
	private BigInteger e;//khóa công khai.
	private BigInteger d;//khóa bí mật.
	...
	public RSAModel(int digits) {
            //tham số là digits, đại diện cho số chữ số trong các số nguyên tố p và q sẽ được tạo ra.
		this.p = generatePrimeNumber(digits);
                //Tạo một số nguyên tố lớn p
		this.q = generatePrimeNumber(digits);
		this.n = p.multiply(q);
                //Tính tích của hai số nguyên tố p và q, lưu kết quả vào biến n. Đây là thành phần modulus của khóa công khai và khóa bí mật.
		this.phiN = findLCM(p.subtract(BigInteger.ONE), q.subtract(BigInteger.ONE));
//                Tính giá trị của hàm Euler (phi) của n, lưu kết quả vào biến phiN. Hàm Euler của một số là số lượng các số nguyên tố tới mà không chia hết cho nó.
		this.e = findPublicExponent(phiN);
                //Tìm một số nguyên tố e (thành phần khóa công khai) sao cho e và phiN là nguyên tố cùng nhau.
		this.d = findModInverse(e, phiN);
                //Tìm số d (thành phần khóa bí mật) sao cho (d * e) % phiN = 1. Đây là bước tính toán để tìm khóa bí mật từ khóa công khai đã được chọn.
	}
...
// d là số mũ riêng, tạo thành khóa riêng.
	...
	public int mod(int a, int n) {
		return (a % n + n) % n;
                //Phương thức này tính toán phép chia lấy phần dư của a cho n.
//Vì kết quả của phép chia có thể là số âm, nên để đảm bảo kết quả là một số không âm, thêm n vào phần dư và lấy phần dư của tổng này khi chia cho n.
	}
